    ...
    async (req, res) => {
        try {
            ...
            const calculatedDuration =
                durationUnit === "hours" ? diffDays * 8 : diffDays;
    ...
    async (req, res) => {
        try {
            ...
            if (leave.employee.toString() !== req.user._id.toString()) {
                return res
                    .status(403)
                    .json({ message: "You can only edit your own leaves" });
            }
...
router.delete("/:id", auth, async (req, res) => {
    try {
        ...
        if (leave.employee.toString() !== req.user._id.toString()) {
            return res
                .status(403)
                .json({ message: "You can only cancel your own leaves" });
        }

        if (leave.status !== "pending") {
            return res
                .status(400)
                .json({ message: "Can only cancel pending leave requests" });
        }
